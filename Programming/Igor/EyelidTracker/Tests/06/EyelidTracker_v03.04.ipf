...
Macro EyelidTracker()
	...
	Variable /G upperEyelid = 0
	Variable /G lowerEyelid = 1
...
Function SelectROIFunc(ctrlName) : ButtonControl
	...
	Make /N=(numFrames) /O wUpperEyelid=0
	Make /N=(numFrames) /O wLowerEyelid=0
	Make /N=(numFrames) /O wUpperEyelidRaw=0
	Make /N=(numFrames) /O wLowerEyelidRaw=0

	// Create LineScanWindow
	DoWindow /K LineScanWindow
	Display /N=LineScanWindow /W=(10, 300, 210, 450) wLineScan
	Label /W=LineScanWindow left "Normalzied Line Integration"
	Label /W=LineScanWindow bottom "Y Coordinate (Pixel)"
	SetAxis /W=LineScanWindow left -0.01, 1
	ModifyGraph /W=LineScanWindow lsize=1.5
	...
	Display /N=TraceWindow /W=(450, 300, 750, 500) wPixelCount, wLineCount
	ModifyGraph rgb(wLineCount)=(0, 0, 65535)
	Label /W=TraceWindow left "Fraction of Eyelid Closure (FEC)"
	Label /W=TraceWindow bottom "Frame Index"
	SetAxis /W=TraceWindow left -0.01, 1
	ModifyGraph /W=TraceWindow lsize=1.5
	
	//=============================================
	// Create EyelidWindow
	DoWindow /K EyelidWindow
	Display /N=EyelidWindow /W=(450, 500, 750,700) wUpperEyelid, wLowerEyelid
	ModifyGraph rgb(wUpperEyelid)=(65535, 0, 0)
	ModifyGraph rgb(wLowerEyelid)=(16384,48896,65280)
	Label /W=EyelidWindow left "Fractioin of Eyelid Closure (FEC)"
	Label /W=EyelidWindow bottom "Frame Index"
	SetAxis /W=EyelidWindow left -0.01, 1
	SetAxis /W=EyelidWindow bottom 0, numFrames
	ModifyGraph /W=EyelidWindow lsize=1.5
	
	//=============================================
	// Create LineScan2DWindow
	DoWindow /K LineScan2DWindow
	NewImage /N=LineScan2DWindow wLineScan2D
	AppendToGraph /W=LineScan2DWindow wUpperEyelidRaw, wLowerEyelidRaw
	ModifyGraph rgb(wUpperEyelidRaw)=(65535, 0, 0)
	ModifyGraph rgb(wLowerEyelidRaw)=(16384,48896,65280)
	...
	ModifyGraph /W=LineScan2DWindow lsize=1.5
...
Function AnalyzeVideoFunc(ctrlName) : ButtonControl
	...
	WAVE wUpperEyelid = root:wUpperEyelid
	WAVE wLowerEyelid = root:wLowerEyelid
	WAVE wUpperEyelidRaw = root:wUpperEyelidRaw
	WAVE wLowerEyelidRaw = root:wLowerEyelidRaw
	
	NVAR upperEyelid = root:upperEyelid
	NVAR lowerEyelid = root:lowerEyelid
	...
	Variable upperEyelid0 = upperEyelid
	Variable lowerEyelid0 = lowerEyelid
	...
	for(frameIndex = 0; frameIndex < numFrames; frameIndex += 1)
		...
		wUpperEyelidRaw[frameIndex] = upperEyelid
		wLowerEyelidRaw[frameIndex] = lowerEyelid
		
		wUpperEyelid[frameIndex] = (upperEyelid - upperEyelid0) / (lowerEyelid0 - upperEyelid0)
		wLowerEyelid[frameIndex] = (lowerEyelid0 - lowerEyelid) / (lowerEyelid0 - upperEyelid0)
...
Function ImageProcessingFunc()
	...
	//===================================
	// upperEyelid and lowerEyelid
	WaveStats /Q wLineScan
